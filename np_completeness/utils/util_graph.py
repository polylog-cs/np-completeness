...
def color_from_potential(weight, pot_dif):
    ...
    if pot_dif > 0:
        ...
        pot_dif_cropped = min(-pot_dif, weight)
        return mcolors.to_hex(
            (pot_dif_cropped / weight) * np.array(mcolors.to_rgb(GREEN))
            + (1.0 - pot_dif_cropped / weight) * np.array(mcolors.to_rgb(GRAY))
        )
...
def edge_potential_updater(mob, edge, graph):
    ...
    pot_dif = (
        graph.vertex_potentials[v].get_value() - graph.vertex_potentials[u].get_value()
    )
...
class CustomGraph(Graph):
    ...
    def create_name(self, vertex, name, offset, scale=0.5):
        self.vertex_names[vertex] = (
            Tex(name, color=GRAY, font_size=DEFAULT_FONT_SIZE)
            .scale(scale)
            .move_to(self.vertices[vertex].get_center())
            .shift(offset)
        )
        self.vertex_names[vertex].add_updater(
            lambda mob, dt: mob.move_to(self.vertices[vertex].get_center()).shift(
                offset
            )
        )
    ...
    def create_edge_length(self, edge, weight, offset=0 * RIGHT):
        number = DecimalNumber(weight, num_decimal_places=1, color=GRAY).scale(0.3)
    ...
    def show_edge_lengths(self, edges):
        anims = []
        for e in edges:
            anims.append(FadeIn(self.edge_weights_objs[e]))
        return AnimationGroup(*anims)
    ...
    def disable_colors(self):
        for e in self.edges:
            self.edges[e].add_updater(lambda mob, dt: mob.set_color(GRAY))
        ...
        for v in self.vertices:
            self.vertex_height_lines[v] = Line(  # DashedLine(
                start=self.vertices[v].get_center(),
                    ...
                    [
                        self.vertices[v].get_center()[0],
                        self.vertices[v].get_center()[1],
                        0,
                    ]
                )
                + 0.001 * DOWN,
        ...
        for edge in self.edges:
            ...
            def edge_arrow_potential_updater(mob, edge):
                ...
                pot_dif = (
                    ...
                    - self.vertex_potentials[u].get_value()
                )
                mob.set_color(color_from_potential(weight, pot_dif))
    ...
    def add_directed_edge(self, u, v, offset=0, weight=1, offset_weight=0):
        ...
        edge.add_updater(
            lambda mob, dt, u=u, v=v, offset=offset: edge_updater(mob, u, v, offset)
        )
    ...
    def run_dijkstra(self, start_node, end_node, speed, thumbnail=False):
                ...
                for neighbor in G[node]:
                    if not neighbor in distances:
                        ...
                        new_dist = (
                            ...
                            + self.vertex_potentials[neighbor].get_value()
                            - self.vertex_potentials[node].get_value()
                        )
        ...
        for anim in mover_anims:
            ...
            if end_time >= distances[end_node]:
                ratio = (
                    (distances[end_node] - start_time) * 1.0 / (end_time - start_time)
                )
