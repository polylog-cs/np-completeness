...
class Wire(VMobject):
    ...
    def update_create(self, mobject, dt):
        if self.activation_animation:
            if self.active:  
                self.activate_progress.set_value(min(self.activate_progress.get_value() + dt * SIGNAL_SPEED / self.length, 1))
            else:
    ...
    def propagate_signal(self, dt):
        if self.value is not None:
            self.progress.set_value(min(self.progress.get_value() + dt * SIGNAL_SPEED / self.length, 1))

    def propagate_signal_back(self, dt):
        if self.value is not None:
            self.progress_back.set_value(min(self.progress_back.get_value() + dt * SIGNAL_SPEED / self.length, 1))
...
class Gate(VMobject):
    def __init__(self, gate_type, inputs=None, output=None, **kwargs):
        ...
        self.text = Text(gate_type).move_to(self.rect.get_center()).scale_to_fit_height(GATE_HEIGHT*GATE_TEXT_RATIO)
    ...
    def update_create(self, mobject, dt):
        if self.activation_animation:
            if not self.active and all(input_wire.activate_progress.get_value() == 1 for input_wire in self.inputs):
                self.active = True
                if self.output:
                    self.output.active = True

            if self.active:  
                self.activate_progress.set_value(min(self.activate_progress.get_value() + dt * SIGNAL_SPEED, 1))
            else:
                self.activate_progress.set_value(0)

            alpha = self.activate_progress.get_value()
            self.rect.scale_to_fit_height(GATE_HEIGHT * alpha + EPSILON)
            self.text.move_to(self.rect.get_center()).scale_to_fit_height(self.rect.height * GATE_TEXT_RATIO)
...
class MultiplicationCircuit(Circuit):
    def __init__(self, **kwargs):
        ...
        for i in range(4):
            for j in range(4):
                and_gate = AndGate()
                and_gate.move_to((i+j)*GATE_HORIZONTAL_SPACING*LEFT + i*GATE_VERTICAL_SPACING*DOWN + 2*UP + 4*RIGHT)
                self.add_gate(and_gate)
                out_wire = Wire(and_gate.get_bottom(), and_gate.get_bottom() + 0.3*GATE_HEIGHT*DOWN)
        ...
        for t in range(2):
            for i in range(4):
                input_wire = Wire(self.gates[i].left_input_position() + t*0.3*GATE_WIDTH*RIGHT + (t+1)*GATE_HEIGHT*UP, 
                                  self.gates[i].left_input_position() + t*0.3*GATE_WIDTH*RIGHT + (t+0.3)*GATE_HEIGHT*UP)
        ...
        for i in range(4):
            for j in range(4):
                ...
                short_wire = Wire(last_wires[j].end_point, self.gates[i*4+j].left_input_position())
                ...
                left_wire = Wire(last_wires[j].end_point, last_wires[j].end_point + (1/3*GATE_WIDTH+1/2*(GATE_HORIZONTAL_SPACING-GATE_WIDTH))*LEFT)
                last_wires[j].add_output_wire(left_wire)
                left_wire.add_input_wire(last_wires[j])
                self.add_wire(left_wire)

                # next wire goes down
                down_wire = Wire(left_wire.end_point, left_wire.end_point + 0.9*GATE_VERTICAL_SPACING*DOWN)
                left_wire.add_output_wire(down_wire)
                down_wire.add_input_wire(left_wire)
                self.add_wire(down_wire)

                # next wire is diagonal
                diagonal_wire = Wire(down_wire.end_point, down_wire.end_point + 0.1*GATE_VERTICAL_SPACING*DOWN + 0.1*GATE_HORIZONTAL_SPACING*LEFT)
                ...
                last_wire = Wire(diagonal_wire.end_point, self.gates[(i+1)*4+j].left_input_position() + 0.3*GATE_HEIGHT*UP)
        ...
        for i in range(4):
            first_wire = Wire(self.input_wires[4+i].end_point, self.gates[4*i].right_input_position() + 0.6*GATE_HEIGHT*UP)
            ...
            left_wires = []
            for j in range(3):
                left_wire = Wire(last_wire.end_point, last_wire.end_point + GATE_HORIZONTAL_SPACING*LEFT)
            ...
            for j, wire in enumerate([first_wire] + left_wires):
                down_wire = Wire(wire.end_point, self.gates[4*i + j ].right_input_position())
...
class CircuitScene(Scene):
    def construct(self):
        ...
        circuit.run_forward(self, [True, False, True, True, False, True, False, True], 30)

        circuit.reset(self)
        self.wait()


        circuit.run_backward(self, [False, False, False, True, True, True, False, True], 30)
